...
def analyze_by_author(entries: List[Dict]) -> Tuple[str, str]:
    ...
    def pipeline_per_author(entries): return "\n".join(
        get_content(entries))  # type: ignore
...
def get_frequencies(tokens, start=0, sample_size=5):
    ...
    sliced = frequencies.most_common(
    )[start: start + sample_size % len(frequencies)]
...
for author, aggregate in entries_by_author:
    ...
    normalized_frequences = [(token, to_percent(count, tokens))
                             for token, count in frequencies[:10]]
